        ...
        void OnGUI()
        {
            ...
            if (!manager.IsClientConnected() && !NetworkServer.active && manager.matchMaker == null)
            {
                if (noConnection)
                {
                    ...
                    if (GUI.Button(new Rect(xpos, ypos, 400, 40), "LAN Client(C)"))
                    {
                        manager.StartClient();
                    }

                    manager.networkAddress = GUI.TextField(new Rect(xpos + 440, ypos, 205, 40), manager.networkAddress);
                    ypos += spacing;

                    if (UnityEngine.Application.platform == RuntimePlatform.WebGLPlayer)
                    {
                        // cant be a server in webgl build
                        GUI.Box(new Rect(xpos, ypos, 400, 45), "(  WebGL cannot be server  )");
                        ypos += spacing;
                    }
                    else
                    {
            ...
            if (!NetworkServer.active && !manager.IsClientConnected() && noConnection)
            {
                ...
                if (UnityEngine.Application.platform == RuntimePlatform.WebGLPlayer)
                {
                    GUI.Box(new Rect(xpos - 5, ypos, 420, 40), "(WebGL cannot use Match Maker)");
                    return;
                }
                ...
                else
                {
                    if (manager.matchInfo == null)
                    {
                        if (manager.matches == null)
                        {
                            ...
                            GUI.Label(new Rect(xpos, ypos, 200, 30), "Room Name:");
                            manager.matchName = GUI.TextField(new Rect(xpos + 100, ypos, 100, 20), manager.matchName);
                            ypos += spacing;

                            ypos += 20;

                            if (GUI.Button(new Rect(xpos, ypos, 400, 30), "Find Internet Match"))
                            {
